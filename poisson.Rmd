...
```{r}
mod4 <- glm(All ~ Moved_since_2005 + as.factor(CAT_City_2), poisson, data, offset = log(nb_person_2014))
summary(mod4)
anova(mod4, test = "LRT")
cbind(exp(coef(mod4)), exp(confint(mod4)))
```
...
```{r}
mod7 <- glm(All ~ as.factor(Urban_Type) + Moved_since_2005, poisson, data, offset = log(nb_person_2014))
summary(mod7)
anova(mod7, test = "LRT")
cbind(exp(coef(mod7)), exp(confint(mod7)))
write.csv(as.data.frame(resid(mod7)), "residuals.csv", quote = FALSE, row.names = FALSE)
```
